      ...
      {tab === 'equally' ? (
        ...
        <div className="mt-6 px-4">
          ...
          <div className="mt-4 text-white flex justify-end">
            <span className='pr-1'>{calculateTotalAmount()}</span> of {Math.abs(amount)}
          </div>
          <div className='text-red-400 fixed float-right'>
            {calculateTotalAmount() > amount ? `Amount exceeds the total amount by ${Math.abs(calculateTotalAmount() - amount)}` : ''}
          </div>
        </div>
      )}
