...
const AddExpense = () => {
  ...
  console.log('addExpense::',user_id,selectedMemberName)
  const validationSchema = Yup.object().shape({
    description: Yup.string().required('Description is required'),
    amount: Yup.number().required('Amount is required').positive('Amount must be positive').integer('Amount must be an integer'),
    date: Yup.date().required('Date is required').max(new Date(), 'Date cannot be in the future'),
  ...
  const initialFormData = JSON.parse(localStorage.getItem('expenseFormData')) || { description: '', amount: '', date: currentDate ,payer_user_id:user_id};
  ...
  return (
    <div className="bg-primaryColor h-screen px-3 flex flex-col items-center">
      ...
      <div className="mt-6">
        <span className='text-lg text-white font-nunito'>
          Paid by <Link to={`/group/${id}/addexpense/paying`} className="bg-white text-black rounded px-2">{selectedMemberName}</Link> and split <Link to={`/group/${id}/addexpense/adjustamount`} className="bg-white text-black rounded px-2">{tab ? tab.toLowerCase() : 'equally'}</Link>
        </span>
      </div>
    </div>
  );
